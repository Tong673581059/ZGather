...
import com.telchina.libskin.entity.SkinItem;
import com.telchina.libskin.util.L;
import com.telchina.libskin.util.ListUtils;
        ...
        for (int i = 0; i < attrs.getAttributeCount(); i++) {//遍历当前View的属性
            ...
            if (attrValue.startsWith("@")) {//也就是引用类型，形如@color/red
                try {
                    ...
                    Log.i("parseSkinAttr", "attrName:" + attrName + " | attrValue:" + attrValue);
                    Log.i("parseSkinAttr", "id:" + id);
                    Log.i("parseSkinAttr", "entryName:" + entryName);
                    Log.i("parseSkinAttr", "typeName:" + typeName);
                    if (mSkinAttr != null) {
